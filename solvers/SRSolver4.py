...
class SRSolver4(BaseSolver):
    def __init__(self, opt):
        super(SRSolver4, self).__init__(opt)
        self.opt = opt
        self.train_opt = opt['solver']
        self.LR = self.Tensor()
        self.LRx = self.Tensor()
        self.HR = self.Tensor()
        self.SR = None
    ...
    def feed_data(self, batch, need_HR=True, is_train=False, is_complex=False):
        input = batch['LR']
        if is_train:
            add_in = batch['LRx']
            self.LRx.resize_(add_in.size()).copy_(add_in)
    ...
    def train_step(self, is_complex=False):
        ...
        for i in range(self.split_batch):
            with torch.autograd.set_detect_anomaly(True):
                ...
                split_LRx = self.LRx.narrow(0, i*sub_batch_size, sub_batch_size)
                split_HR = self.HR.narrow(0, i*sub_batch_size, sub_batch_size)
                if is_complex:
                    split_LRx = self.LR_x.narrow(0, i*sub_batch_size, sub_batch_size)

                output = self.model(split_LR, split_LRx, is_test=False)    #res_n, feat_n, res_x, feat_x
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.self_ensemble and not self.is_train:
                ...
                SR = forward_func(self.LR, n_GPUs=2, is_test=True)
    ...
    def _overlap_crop_forward(self, x, shave=10, min_size=100000, bic=None, n_GPUs=2, is_test=True):
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 4, n_GPUs):
                ...
                sr_batch_temp = self.model(lr_batch, 0, is_test=is_test)
    ...
    def save_current_log(self):
        data_frame = pd.DataFrame(
            data={'train_loss': self.records['train_loss']
                , 'val_loss_1': self.records['val_loss_1']
                , 'psnr_1': self.records['psnr_1']
                , 'ssim_1': self.records['ssim_1']
                , 'val_loss_2': self.records['val_loss_2']
                , 'psnr_2': self.records['psnr_2']
                , 'ssim_2': self.records['ssim_2']
                , 'val_loss_3': self.records['val_loss_3']
                , 'psnr_3': self.records['psnr_3']
                , 'ssim_3': self.records['ssim_3']
