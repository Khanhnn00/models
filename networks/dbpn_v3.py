...
class D_DBPN_V3(nn.Module):
    def __init__(self,in_channels, out_channels, num_features, bp_stages, upscale_factor=4, norm_type=None, act_type='prelu'):
        super(D_DBPN_V3, self).__init__()
    ...
    def forward(self, x, is_test=False):
        if is_test == False:
            ...
            x = self.feature_extract_1(x)
            x_noise = self.feature_extract_1(x_noise.float())

            feat_x = self.feature_extract_2(x)
            feat_noise = self.feature_extract_2(x_noise)

            x = self.bp_units(feat_x)
            x_noise = self.bp_units(feat_noise)

            x = self.conv_hr(x)
            x_noise = self.conv_hr(x_noise)

            return x_noise, feat_noise, x, feat_x
        else:
            x = self.feature_extract_1(x)
            x = self.feature_extract_2(x)
            x = self.bp_units(x)
